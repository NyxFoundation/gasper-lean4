...
$$`\operatorname{finalization\_fork}(\sigma) \;\Longrightarrow\; \operatorname{q\_intersection\_slashed}(\sigma)`$$
...
$$`\exists\,\sigma',\;\; \operatorname{unslashed\_can\_extend}(\sigma,\sigma') \;\wedge\; \operatorname{no\_new\_slashed}(\sigma,\sigma') \;\wedge\; \operatorname{finalized}(\sigma')`$$
...
$$`\max\!\bigl(\operatorname{wt}(V_L) - a_L - e_R,\;\operatorname{wt}(V_R) - a_R - e_L\bigr) - f_{1/3}\!\bigl(\operatorname{wt}(V_L)\bigr) - f_{1/3}\!\bigl(\operatorname{wt}(V_R)\bigr) \;\le\; \operatorname{wt}(q_L \cap q_R)`$$
...
- **Theories** -- The three main theorems above, with Fitch-style
  proof trees rendered via `#detail_explode`.
-/
